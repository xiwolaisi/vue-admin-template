        ...
        <el-table
          ...
          <el-table-column label="Name" width="350">
            <template slot-scope="scope">
              <p>{{ scope.row.Name }}</p>
              <p>
                <el-tag type="success" v-for="sub in scope.row.Subject">{{sub.name}}({{sub.kind}})</el-tag>
              </p>
            </template>
          </el-table-column>
...
import { getRoleBindingsByNs } from "@/api/rolebindings";
...
export default {
  ...
  methods: {
    ...
    changeNs(ns) {
      this.namespace = ns;
      this.loadRoleBindings(ns);
    },
    loadRoleBindings(ns) {
      getRoleBindingsByNs(ns).then(rsp => {
        this.$set(this.rolebindings, ns, rsp.data);
      });
    }
  }
};
