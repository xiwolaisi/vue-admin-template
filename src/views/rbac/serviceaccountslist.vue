    ...
    <el-container v-if="namespace" :key="namespace" class="namespace-container">
      <el-main>
        <el-table
          ...
          <el-table-column label="ServicAccount" width="350">
            <template slot-scope="scope">
              <p>{{ scope.row.Name }}</p>
            </template>
          </el-table-column>
    ...
    initializeWebSocket() {
      ...
      this.wsClient.onmessage = (e) => {
        if (e.data !== 'ping') {
          ...
          if (object.type === 'serviceaccounts') {
            this.$set(this.sas, object.result.ns, object.result.data);
          }
        }
      };
    },
